...
df_covdist <- df_distances[, c(1, 3:6, 9:15)]
...
# merge df_covdist with df_colera.merged.month as df_colera_inla7

df_colera_inla7 <- merge(df_colera.merged.month, df_covdist, by.x = CODIGO_INE_STR, by.y = "COD_INE")
df_colera_inla7 <- df_colera_inla7[, c(1, 8:9, 2, 3:7, 12:18, 10:11)]
colnames(df_colera_inla7)[c(2:3, 17:18)] <- c(PROVINCIA_STR, MUNICIPIO_STR, LONG_STR, LAT_STR)
...
res_table <- data.frame(matrix(ncol = 5, nrow = 9))
...
res_table[2,1] <- COVPROV_STR
res_table[3,1] <- COVSTATION_STR
res_table[4,1] <- COVRAIL_STR
res_table[5,1] <- COVRIVER_STR
res_table[6,1] <- COVROAD_STR
res_table[7,1] <- COVCOAST_STR
res_table[8,1] <- COVPORT_STR
res_table[9,1] <- "Unstructured random effect"
